...
with tab4:
        ...
        if uploaded_file:
            ...
            if file_size_mb > document_service.MAX_UPLOAD_SIZE_MB:
                ...
                doc_description = st.text_area(
                    "Description", help="Brief description of document content", height=80
                )
                ...
                doc_type = st.selectbox(
                    "Document Type", options=doc_types, help="Classify the type of document"
                )
                    ...
                    with st.container(border=True):
                        ...
                        with col_name:
                            ...
                            if doc.get("description"):
                                st.caption(
                                    doc["description"][:100]
                                    + ("…" if len(doc["description"]) > 100 else "")
                                )
    ...
    with doc_tab3:
        ...
        csv_file = st.file_uploader(
            "Upload CSV file", type=["csv"], help="CSV file with document metadata and file paths"
        )
